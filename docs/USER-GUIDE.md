...
Controls the update check behavior. When enabled (the default), loom checks for updates once per hour on startup and notifies you if a newer version is available. Run `loom update` to install.
