...
Loom checks for updates hourly on startup and notifies you when a new version is available. Run `loom update` to install. To disable: set `LOOM_DISABLE_UPDATE=1` or add `[update] enabled = false` to config.
